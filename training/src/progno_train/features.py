...
def _build_player_index(history: pd.DataFrame) -> dict[int, pd.DataFrame]:
    ...
    Each frame contains all matches for that player (won=True/False, opponent_rank),
    sorted by tourney_date.  Use _slice_before() to time-gate before feature calls.
    """
    if history.empty:
        return {}
...
def _player_matches_before(
    ...
    """All matches for player_id strictly before as_of_date (full-scan fallback)."""
    if history.empty:
        return pd.DataFrame()
...
def rolling_win_rate(
    ...
    *,
    ...
    Pass _frame (already sliced to < as_of_date) to skip the full-history scan.
    """
...
def compute_match_features(
    ...
    *,
    ...
    Pass _frame_a/_frame_b (pre-sliced to < tourney_date) to avoid full-history scans.
    """
    feats: dict[str, Any] = {}
    ...
    # H2H (still uses full history — pair-specific, not per-player)
...
def build_all_features(
    ...
    for _, row in history[history["is_complete"]].iterrows():
        ...
        feats_pos = compute_match_features(
            ...
            _frame_a=fa, _frame_b=fb,
            **common_kwargs,
        )
        feats_pos["label"] = 1
        feats_pos["tourney_date"] = date
        feats_pos["year"] = date.year
        ...
        feats_neg = compute_match_features(
            ...
            _frame_a=fb, _frame_b=fa,
            **common_kwargs,
        )
        feats_neg["label"] = 0
        feats_neg["tourney_date"] = date
        feats_neg["year"] = date.year
        rows.append(feats_neg)

        if len(rows) % 20000 == 0 and len(rows) > 0:
            log.info("build_all_features: processed %d rows...", len(rows))
